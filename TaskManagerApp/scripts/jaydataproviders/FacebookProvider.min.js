...
encodeURIComponent(c.queryText)+"&"+this.providerConfiguration.FQLFormat;this.providerConfiguration.Access_Token&&(c+="&access_token="+this.providerConfiguration.Access_Token);c={url:c,dataType:"JSON",success:function(c){a.rawDataList=c.data;Container.createModelBinderConfigCompiler(a,[]).Visit(a.expression);b.success(a)},error:function(a,c,e){c={};try{c=JSON.parse(a.responseText).error}catch(f){c=e+": "+a.responseText}b.error(c)}};this.context.prepareRequest.call(this,c);this.AuthenticationProvider.CreateRequest(c)},
_discoverType:function(a,b,c){b.memberDefinitions.getPublicMappedProperties().forEach(function(b){var d=Container.resolveType(b.dataType);if(d.isAssignableTo||d==Array){var f=a?a+"."+b.name:b.name;if(d==Array||d.isAssignableTo($data.EntitySet))if(b.inverseProperty)d=Container.resolveType(b.elementType);else return;c.push({name:f,type:d});this._discoverType(f,d,c)}},this)},_compile:function(a){return Container.createFacebookCompiler().compile(a)},getTraceString:function(a){this.AuthenticationProvider||
(this.AuthenticationProvider=new $data.Authentication.Anonymous({}));return this._compile(a)},setContext:function(a){this.context=a},saveChanges:function(){Guard.raise(new Exception("Not implemented","Not implemented"))}},null);$data.StorageProviderBase.registerProvider("Facebook",$data.storageProviders.Facebook.FacebookProvider);
